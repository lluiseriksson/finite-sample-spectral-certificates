...
\hypersetup{pdftitle={Projective Bottlenecks in Passive Spectral Routing},pdfauthor={Lluis Eriksson},pdfsubject={Exact planar memory, quantitative approximation barriers, and projective strata}}
...
Exact Planar Memory, Quantitative Approximation Barriers, and Cross-Ratio Strata}
...
barrier for low-state routers, an exact zero-memory minimax law, the resulting
generic law, unbounded detector gap, and complete four-line stratification.
...
output line extends to a constant unitary.  If its Bloch vector is $\mathbf r$,
then
\[
...
full minimax problem only at degree zero; determining $E_d$ exactly for general
$d\ge1$ below the exact-routing threshold remains open.
\end{limitation}
...
physical/detector consequences.  Bloch-sphere cap geometry itself is standard;
...
\begin{center}\small
\href{https://github.com/lluiseriksson/finite-sample-spectral-certificates/releases/tag/v3.2-planar-projective-memory}{GitHub release: \texttt{v3.2-planar-projective-memory}}
\end{center}
...
collision asymptotics for two targets.  The analysis also marks the next frontier precisely:
