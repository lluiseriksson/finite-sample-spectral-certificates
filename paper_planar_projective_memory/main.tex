...
\[
 ...
 \]
so it is computable in polynomially many exact field operations, without a
subset search.  Equivalently, $E_d=0$ exactly when the minimum modulus
$\inf_{\|c\|=1}\|M_dc\|$ vanishes.  Letting $\delta(A)$ denote the projective
...
\begin{corollary}[Polynomial-time exact border certificate]\label{cor:border-certificate}
The integer $d_\partial$ is obtained by scanning
$M_0,M_1,\ldots,M_{\lfloor L/2\rfloor}$ and stopping at the first loss of full
column rank.  Dense exact elimination uses $O(L^4)$ field operations in this
direct implementation.  A nonzero kernel vector at the stopping degree is an
...
elimination on an $L\times O(L)$ matrix costs $O(L^3)$ field operations and
there are $O(L)$ candidate degrees.  The certificate statements are exactly
...
the resulting polynomial state certificate, together with the passive-memory
...
and its polynomial-time rank form \eqref{eq:border}, and the sharp universal closure theorem
...
\begin{center}\small
\href{https://github.com/lluiseriksson/finite-sample-spectral-certificates/releases/tag/v3.5-planar-projective-memory}{GitHub release: \texttt{v3.5-planar-projective-memory}}
\end{center}
...
thereby classifies all three regimes exactly.  The same boundary is the first
degree at which the interpolation matrix loses full column rank, replacing an
exponential subset formula by an $O(L^4)$ exact algorithm and unifying the
