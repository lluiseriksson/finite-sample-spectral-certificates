...
$d_\partial=\min\{d:E_d=0\}$ and let $\delta(A)$ be the projective interpolation
degree of the subtable indexed by $A$.  We prove the base-point deletion law
\[
...
barrier for low-state routers, exact zero-memory and base-point deletion laws,
...
\begin{theorem}[Exact base-point deletion law]\label{thm:border}
Every finite planar routing table satisfies
\begin{equation}\label{eq:border}
 \boxed{\displaystyle
 d_\partial=\min_{B\subseteq[L]}
                  \bigl\{|B|+\delta(B^c)\bigr\}.}
\end{equation}
...
\]
This proves that the right side of \eqref{eq:border} is no larger than
$d_\partial$.
...
zero is detected by the homogeneous degree-$D$ resultant.  Since
...
lossless lift proves $E_{b+r}=0$, and then \eqref{eq:border}.
...
\begin{corollary}[Finite exact certificate for border memory]\label{cor:border-certificate}
The integer $d_\partial$ is decidable by finite exact linear algebra and
polynomial gcd tests.  Explicitly, for each $A\subseteq[L]$, compute
$\delta(A)$ from the first interpolation matrix $M_d(A)$ whose kernel contains
a coprime pair, and minimize $L-|A|+\delta(A)$.  A minimizing subset and its
coprime kernel pair certify the upper bound; the rank/subresultant exclusions
for all smaller subset costs certify the lower bound.
\end{corollary}

\begin{proof}
Proposition~\ref{prop:certificate} computes every $\delta(A)$ using
$0\le d\le |A|-1$, and \cref{thm:border} gives the stated minimum.  All tests
are exact over the coefficient field of the data.
\end{proof}
...
algebraic mechanism.  We claim the passive-memory equality \eqref{eq:main}, obtained by
...
\eqref{eq:border}, and the sharp universal closure theorem
...
\begin{center}\small
\href{https://github.com/lluiseriksson/finite-sample-spectral-certificates/releases/tag/v3.4-planar-projective-memory}{GitHub release: \texttt{v3.4-planar-projective-memory}}
\end{center}
...
thereby classifies all three regimes exactly.  Its binary specialization gives
