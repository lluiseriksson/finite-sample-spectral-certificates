...
group-delay design are also established \cite{BharathEtAl2023}.  Within the
sources compared in \cref{tab:precedent}, we did not find the exact
orthogonal fan-out specialization, its scalable pairwise/global gap, or the
operator-norm robust inertia certificate.  Those are the contributions
here; no new general Pick theorem is claimed.
...
For completeness, a feasible rank-$r$ matrix factors as
$P_{ij}=F_i^*F_j$ with $F_i\in\C^{r\times k}$.  The two families
\[
 \begin{pmatrix}\zeta_iF_iu\\Xu\end{pmatrix},
 \qquad
 \begin{pmatrix}F_iu\\W_iu\end{pmatrix}
\]
have identical Gram matrices.  Their correspondence extends to a unitary
colligation on $\C^r\oplus\C^N$, whose transfer interpolates the frames.
Any unit-circle state mode is reducing and decoupled and can be deleted.
Conversely, the compressed de Branges--Rovnyak kernel of any degree-$n$
interpolant supplies such a completion of rank at most $n$.  This is the
classical lurking-isometry argument behind \cref{prop:pick-rank}.
...
\href{https://github.com/lluiseriksson/finite-sample-spectral-certificates/releases/tag/v2.0-global-fanout-memory}{immutable GitHub release}.  The main entry points are
