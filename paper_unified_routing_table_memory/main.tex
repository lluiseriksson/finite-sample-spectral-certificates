...
fan-out, and repeated orthogonal routing as special cases.  We also give a
universal span sandwich outside the direct-sum locus, an open-dense
genericity statement, a fail-closed noisy certificate, an exact collision
discontinuity, the complete three-line singular phase diagram, and the
optimal integrated Wigner--Smith delay.  A producer and an independent
verifier audit 512 nonorthogonal scalar tables, 256 block tables, and a
near-collision sweep.  All source, frozen evidence, and release metadata are
linked visibly in the paper.
\end{abstract}
...
Apply \cref{lem:compiler-degree} with $d=L-n_*$.  The completed square
...
statistic alone ceases to be complete.  Put
$r=\dim\sum_{a\in\A(w)}Y_a$.
...
\begin{proof}
For a minimal realization $S(z)=D+zC(I-zA)^{-1}B$ of degree $q$, the
...
The lower bound is exact on the all-distinct direct-sum stratum, but need
not be exact on singular strata.  The smallest complete example is useful
enough to state.
...
\begin{proof}
A degree-one rational-inner matrix is, up to constant unitaries, one
rank-one Blaschke--Potapov factor.  Its orbit of an input line is constant or
injective and contained in a two-plane.  This excludes repeated-but-not-
constant triples and triples spanning dimension three.  Conversely, three
distinct lines in a two-plane correspond on the Bloch sphere to three
points on one non-tangent latitude after a rotation.  A disk automorphism
maps the three boundary nodes, with matching cyclic orientation, to their
three latitude phases; the associated degree-one Potapov factor realizes
the data.  The universal upper bound supplies degree two in the remaining
cases.
\end{proof}
...
Independent fail-closed replay & \path{verification/verify_unified_routing_table_memory.py}\\
...
The new result is not any one of those ingredients.  It is the exact
direct-sum occupancy identity \eqref{eq:master}, including its dual-minor
necessity and its target-weighted matrix spectral compiler.  A targeted
search of the primary literature through 11 August 2026 did not locate this
formula or the statement that the least occupancy is a complete generic
state statistic.  This supports a priority claim for the precise theorem,
...
Outside direct-sum position, \cref{eq:sandwich} remains valid but occupancy
alone need not determine degree; \cref{thm:three-lines} is the smallest
counterexample.  The paper does not claim a complete formula for every
intersection lattice of target subspaces.
\end{limitation}
...
The next mathematical frontier is therefore sharply defined: replace the
direct-sum hypothesis by an intersection lattice and identify which families
of dual minors can share one state budget.  The next physical frontier is a
robust approximate law coupling node separation, projector error, and
allowed leakage.  Neither problem is concealed inside the present exact
claim.

\section{Conclusion}

A finite spectral routing table has a simple generic memory law.  Direct-sum
geometry creates a dual detector for every target; every absence consumes
$k$ determinant zeros, so the least visited target asks for the largest
budget.  A matrix Fej\'er--Riesz compiler shares exactly that budget across
all detectors.  The equality
\[
                         d_{\min}=k(L-n_*)
\]
therefore unifies orthogonal fan-out, generic fan-out, and routing words,
while the collision and three-line results show precisely why no universal
occupancy formula survives on all singular strata.  The same integer is at
once a state count, a determinant-winding count, and the optimal integrated
Wigner--Smith action divided by $2\pi$.

\paragraph{Immutable reproducibility record.}
Full release URL (tag included):
\begin{center}\small
\url{https://github.com/lluiseriksson/finite-sample-spectral-certificates/releases/tag/v2.6-unified-routing-table-memory}
\end{center}
The release records the exact target commit, PDF SHA-256, producer command,
independent verifier, and CI status.  The three predecessor preprints remain
citable historical records; this article is the unified extension and does
not silently replace their artifacts.
