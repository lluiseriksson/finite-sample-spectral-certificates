...
still gives rigorous information.  For $D\in\C^{k\times N}$ put
\[
 \rho_a(D)=\rank(DW_a),
\]
which is independent of the chosen frame of $Y_a$.  Call $D$ admissible if
$\max_a\rho_a(D)=k$, and define
\begin{equation}\label{eq:detector-rank-statistic}
 \Delta(\mathcal Y,n)=
 \max_{D\ {\rm admissible}}
       \sum_{a\in\A(w)} n_a\bigl(k-\rho_a(D)\bigr).
\end{equation}
...
\begin{proof}
Fix an admissible $D$ and set $g_D(z)=\det(D S(z)X_0)$.  At a node labelled
$a$, the endpoint gauge is invertible, hence
$\rank(D S(\zeta_i)X_0)=\rho_a(D)$.  If an analytic $k\times k$ matrix has
rank $q$ at $z_0$, constant row and column operations reduce its value to
$\operatorname{diag}(I_q,0)$; every nonzero determinant term must then use
at least $k-q$ entries vanishing at $z_0$.  Therefore
\[
 \operatorname{ord}_{\zeta_i}g_D\ge k-\rho_a(D).
\]
Admissibility makes $g_D$ nonzero at at least one interpolation node, so the
compressed-minor budget in \cref{lem:budget} gives
\[
 \mcdeg S\ge\sum_a n_a\bigl(k-\rho_a(D)\bigr).
\]
Maximize over $D$ and combine with the realization-span argument in
...
diagram is nevertheless fully controlled by \cref{cor:hyperplane}.
...
pairwise distinct coplanar lines, \cref{cor:hyperplane} gives
$d_{\min}\ge1$ and \cref{lem:cp1} attains one.  If the three lines span
...
\[
                 \sum_a n_a\bigl(k-\rank(D|_{Y_a})\bigr).
\]
...
\begin{center}\small
\url{https://github.com/lluiseriksson/finite-sample-spectral-certificates/releases/tag/v2.7-detector-rank-hierarchy}
\end{center}
The release records the exact target commit, PDF SHA-256, producer commands,
independent verifiers, and CI status.  Release v2.7 supersedes v2.6 as the
strengthened artifact of this same unified paper.  The three predecessor
