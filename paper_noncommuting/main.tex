...
asymptotic separation: at degree $N$ and channel dimension $d=N$, there are
...
$N\ge5$, with $d=N$ and $M=2N$, one can choose distinct rational pass
...
\end{enumerate}
For every $N\ge5$, the coefficient matrices of $P_N$ cannot
commute pairwise.
\end{theorem}
...
Let $d=N$ and assign two nodes to every coordinate group.  One concrete
choice, with $1\le r\le N$, is
\begin{equation}\label{eq:pairednodes}
 a_r=1+\frac{r}{10N}, \qquad b_r=2+\frac{r}{10N}.
\end{equation}
These $2N$ nodes are distinct, lie in $[1,7/2]$, and have mutual separation
at least $(10N)^{-1}$.

Write $T_m$ for the Chebyshev polynomial of the first kind and set
$\phi(x)=2x+1$, which maps $I_-$ onto $[-1,1]$.  For a coordinate group with
nodes $a\ne b$, define
\begin{align}
 q_{a,b}(x)
 ...
 \label{eq:twopoint}
\end{align}
These polynomials equal one at both assigned nodes.  Put them on the diagonal
of $P_0(x)$ according to the coordinate partition.
...
\]
In \eqref{eq:pairednodes}, $b_r-a_r=1$.  On $I_-$, the sum of the absolute
...
separation is at least $(10N)^{-1}$, while any two points in the stop/pass
domain are at distance at most $9/2$.  A Lagrange basis polynomial on at most
four nodes is bounded by $(45N)^3$.  Summing at most four basis polynomials and
using $\|A\|_{\op}\le d\max_{r,s}|A_{rs}|$ gives
\begin{equation}\label{eq:rightinversebound}
 \|R_N\|_{Y\to X}\le4(45N)^3d=364500N^4.
\end{equation}

Write $L_\varepsilon=L_0+\varepsilon K_N$.  Since
$\|w_j\|_2\le\sqrt d=\sqrt N$, $\|K_N\|_{X\to Y}\le\sqrt N$.  Hence
\[
 L_\varepsilon R_N=I+E_N,\qquad
 \|E_N\|\le364500\varepsilon N^{9/2}.
\]
For $\varepsilon=\varepsilon_N$ and $N\ge5$, this bound is below $1/2$:
its logarithmic derivative is $9/(2N)-3N^2<0$, while at $N=5$ it is
below $364500\,5^5 2^{-125}<2^{-94}$.  Hence the inverse
...
\[
 \|r_N\|_Y\le\varepsilon_N\sqrt N(1+10e^{N\Delta\eta})
 \le11\varepsilon_N\sqrt N e^{N\Delta\eta}.
\]
Set
\[
...
\[
 \|\Delta P_N\|_X
 \le2(364500N^4)(11\varepsilon_N\sqrt N e^{N\Delta\eta})
 =8019000N^{9/2}e^{-N^3+N\Delta\eta}.
\]
...
The ratio of the correction bound to the base bound is
\[
 \frac{8019000}{52/5}N^{9/2}e^{-N^3+N\eta_1-\eta_0};
\]
its logarithmic derivative is $9/(2N)-3N^2+\eta_1<0$, since
$\eta_1=\log(8+\sqrt{63})<\log16<3$.  At $N=5$, using $e>2$, the displayed
ratio is below $2^{-78}$.  Thus the correction in
Lemma~\ref{lem:stability} is at most
$(52/5)e^{\eta_0}e^{-\eta_0N}$.  The first conclusion holds
with
\[
 C=\frac{104}{5}e^{\eta_0},\qquad c=\eta_0.
\]
The second conclusion is Lemma~\ref{lem:commuting}.  Finally,
$e^{\eta_0}=3+\sqrt8>5$ gives
$Ce^{-cN}\le(104/5)e^{-4\eta_0}<1$ for $N\ge5$, so the constructed
coefficients cannot commute pairwise.
\end{proof}
...
For every $N\ge5$ there are $M=2N$ distinct frequencies
$\omega_j\in[0,\arccos(-1/9)]$ and full-spark directions
$v_j\in\R^{N}$ for which a causal response of the form
...
\href{https://github.com/lluiseriksson/finite-sample-spectral-certificates/blob/research/endogenous-visibility/verification/verify_linear_dimension_scaling.py}{linear-dimension theorem arithmetic replay},
the
