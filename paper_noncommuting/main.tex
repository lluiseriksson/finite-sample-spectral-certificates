...
error on six held-out records is $0.00385$.  Numerical programs test the
...
This is a calibration experiment, not a fault classifier.  We selected twelve
...
Thus the measured instance closes the purely constructed-data objection: its
calibration and validation use disjoint sensor records, and the exact $0.96$-versus-$1$
gap is not solver-dependent.  It does not show that a deployed pump must impose
these exact directional constraints, nor that the filter improves diagnosis,
latency or hardware performance.
...
Those works use rational transfer-function approximation and do not provide a
fixed-degree Hermitian-polynomial separation from the simultaneously
diagonalizable subclass.  Minimax polynomial filtering and SDP
...
\item strengthen the measured calibration into a task-level validation in
      which directional preservation and fixed delay are mandated rather than
      inferred from a normal-condition spectral summary.  The present split,
      approximate-calibration controls and unconstrained MIMO baseline close
      the data-grounding gate but not a deployment claim.
\end{enumerate}
