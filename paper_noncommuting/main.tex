...
\title{Exponential Calibration Laws for Noncommuting\\
...
a symmetric noncommuting filter has stopband leakage $O(e^{-cN})$, whereas
every exactly feasible pairwise-commuting symmetric filter is the identity.
The signature margin and the admissible commuting calibration error are both
$e^{-O(N)}$, not $e^{-N^3}$.  This exponential scale is unavoidable: for
...
noncommuting construction achieves the correct exponential scale class.  An
...
as new; the contribution is the fixed-order commuting/noncommuting separation
...
channel mixing to depend on polynomial degree can reduce \eqref{eq:leakage}
under the same pass constraints, and at what calibration-error scale that
comparison remains meaningful.  For unit signatures define
\[
 \gamma(v_1,\ldots,v_M)
...
spark and have an explicit $e^{-O(N)}$ lower margin.  This makes the commuting
obstruction quantitative under $e^{-O(N)}$ residuals, while a separate scalar
construction proves that no asymptotic advantage can survive calibration
errors bounded below independently of $N$.
...
\item More generally, if such a commuting $Q_N$ obeys
\[
 \max_j\|(Q_N(\lambda_j)-I)v_j\|_2\le\delta,
\]
then, with $L_N=(24e/5)^N$,
\begin{equation}\label{eq:robustmain}
 \rho(Q_N)\ge1-\frac{\sqrt3L_N}{\underline\gamma_N}\,\delta.
\end{equation}
Consequently $\rho(Q_N)\ge3/4$ whenever
$\delta\le\underline\gamma_N/(4\sqrt3L_N)$.
\end{enumerate}
For every $N\ge N_0$, the coefficient matrices of $P_N$ cannot
commute pairwise.
\end{theorem}

The admissible error in the third item is $e^{-O(N)}$.  We split the proof into
an obstruction, a small-leakage base construction, a quantitatively full-spark
rational perturbation, and a matching scale obstruction.
...
\begin{lemma}[Quantitative commuting obstruction]\label{lem:robustcommuting}
For the nodes and signatures above, let $Q$ be a degree-$N$ polynomial with
pairwise-commuting real-symmetric coefficients and suppose
\[
 \max_j\|(Q(\lambda_j)-I)v_j\|_2\le\delta.
\]
Then \eqref{eq:robustmain} holds.
\end{lemma}

\begin{proof}
Simultaneously diagonalize the coefficients and fix a common unit eigenvector
$u$, with scalar polynomial $q$.  If three signatures all had
...
\[
 |q(\lambda_j)-1|\le\frac{\sqrt3\delta}{\underline\gamma_N}.
\]
...
$|q(0)-1|\le\sqrt3L_N\delta/\underline\gamma_N$.  Applying this to the common
eigenbasis gives \eqref{eq:robustmain} because $0\in I_-$.
\end{proof}
...
for $N\ge N_0$, and the robust comparison is strict at the displayed
calibration radius.
\end{proof}
...
If its phase-corrected calibration residual is at most $\delta$, its
high-frequency norm is at least the right-hand side of
\eqref{eq:robustmain}.  Conversely, for arbitrary calibration directions and
...
Lemma~\ref{lem:robustcommuting} transfers the residual bound, and
...
\[
 ...
 \ge 1-60\delta.
\]
In particular, if $\delta<7/1920$, its leakage is strictly larger than the
certified $25/32$ leakage of the noncommuting witness.
\end{proposition}
...
distinct pass nodes
\[
 |q(\lambda_j)-1|\le5\sqrt2\,\delta.
\]
For each of the four possible three-node subsets of
$\{1/2,1,3/2,7/2\}$, the sum of the absolute Lagrange weights at $x=0$ is
at most eight.  Hence $|q(0)-1|\le40\sqrt2\delta<60\delta$.  This holds for
both common eigenvectors, so $\|Q(0)\|_{\op}\ge1-60\delta$.  Finally,
...
on $\omega\in[\arccos(-5/9),\pi]$.  Every commuting symmetric palindromic
...
its high-frequency norm is at least $1-60\delta$.
...
subclass.  The new quantitative result also identifies the calibration scale:
an exponentially small residual is sufficient for the constructed separation,
whereas a universal scalar construction proves that an exponentially small
residual can erase every such separation.  We have not found this two-sided
law in the sources above.
