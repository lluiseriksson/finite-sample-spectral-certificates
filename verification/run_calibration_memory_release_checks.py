#!/usr/bin/env python3
"""Replay and hash-check the topological calibration-memory paper release."""
...
MANIFEST = ROOT / "programme" / "CALIBRATION_MEMORY_ARTIFACT.json"
...
def main() -> None:
    ...
    subprocess.run(
        [sys.executable, "verification/verify_calibration_memory.py"],
        cwd=ROOT,
        check=True,
    )
    with tempfile.TemporaryDirectory(prefix="calibration-memory-replay-") as directory:
        temporary = Path(directory)
        output = temporary / "certificate.json"
        figure = temporary / "memory_law.pdf"
        subprocess.run(
            [
                sys.executable,
                "research/calibration_memory_certificate.py",
                "--output", str(output),
                "--figure", str(figure),
            ],
            cwd=ROOT,
        ...
        frozen = json.loads((ROOT / "results/calibration_memory/certificate.json").read_text(encoding="utf-8"))
    ...
    pdf = ROOT / "paper_calibration_memory/lossless_calibration_memory.pdf"
    if not pdf.read_bytes().startswith(b"%PDF-"):
        raise RuntimeError("paper artifact is not a PDF")
    check_hashes(manifest["artifacts"])
    print("PASS calibration-memory release gate")
