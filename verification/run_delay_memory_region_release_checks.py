#!/usr/bin/env python3
"""Replay and hash-check the complete delay-memory region release artifact."""
...
MANIFEST = ROOT / "programme" / "DELAY_MEMORY_REGION_ARTIFACT.json"
...
def main() -> None:
    ...
    subprocess.run(
        [sys.executable, "verification/verify_delay_memory_regions.py"],
        cwd=ROOT,
        check=True,
    )
    with tempfile.TemporaryDirectory(prefix="delay-memory-region-replay-") as directory:
        temporary = Path(directory)
        output = temporary / "certificate.json"
        figure = temporary / "complete_region.pdf"
        subprocess.run(
            [
                sys.executable,
                "research/delay_memory_region_certificate.py",
                "--output", str(output),
                "--figure", str(figure),
            ],
            cwd=ROOT,
        ...
        frozen = json.loads(
            (ROOT / "results/delay_memory_regions/certificate.json").read_text(encoding="utf-8")
        )
    ...
    pdf = ROOT / "paper_delay_memory_regions/complete_delay_memory_regions.pdf"
    if not pdf.read_bytes().startswith(b"%PDF-"):
        raise RuntimeError("paper artifact is not a PDF")
    check_hashes(manifest["artifacts"])
    print("PASS complete delay-memory region release gate")
