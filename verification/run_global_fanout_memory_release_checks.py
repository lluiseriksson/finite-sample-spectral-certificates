#!/usr/bin/env python3
"""Replay and hash-check the global fan-out memory release artifact."""
...
MANIFEST = ROOT / "programme" / "GLOBAL_FANOUT_MEMORY_ARTIFACT.json"
...
def main() -> None:
    ...
    subprocess.run(
        [sys.executable, "verification/verify_global_fanout_memory.py"],
        cwd=ROOT,
        check=True,
    )
    with tempfile.TemporaryDirectory(prefix="global-fanout-replay-") as directory:
        temporary = Path(directory)
        output = temporary / "certificate.json"
        figure = temporary / "global_fanout_memory.pdf"
        subprocess.run(
            [
                sys.executable,
                "research/global_fanout_memory_certificate.py",
                "--output", str(output),
                "--figure", str(figure),
            ],
            cwd=ROOT,
        ...
        frozen = json.loads(
            (ROOT / "results/global_fanout_memory/certificate.json").read_text(encoding="utf-8")
        )
    ...
    pdf = ROOT / "paper_global_fanout_memory/global_fanout_memory.pdf"
    if not pdf.read_bytes().startswith(b"%PDF-"):
        raise RuntimeError("paper artifact is not a PDF")
    check_hashes(manifest["artifacts"])
    print("PASS global fan-out memory release gate")
