#!/usr/bin/env python3
"""Replay and hash-check the robust spectral-routing paper release."""
...
MANIFEST = ROOT / "programme" / "ROUTING_SPEED_LIMIT_ARTIFACT.json"
...
def compare(expected: object, observed: object, location: str = "root") -> None:
    if isinstance(expected, bool) or isinstance(expected, str) or expected is None:
        ...
        if not isinstance(observed, (int, float)) or not math.isclose(
            float(observed), float(expected), rel_tol=2.0e-8, abs_tol=5.0e-10
        ):
        ...
        raise TypeError(f"unsupported value at {location}: {type(expected)}")
...
def main() -> None:
    ...
    subprocess.run(
        [sys.executable, "verification/verify_routing_speed_limit.py"],
        cwd=ROOT,
        check=True,
    )
    with tempfile.TemporaryDirectory(prefix="routing-speed-limit-replay-") as directory:
        temporary = Path(directory)
        output = temporary / "certificate.json"
        figure = temporary / "routing_law.pdf"
        subprocess.run(
            [
                sys.executable,
                "research/routing_speed_limit_certificate.py",
                "--output", str(output),
                "--figure", str(figure),
            ],
            cwd=ROOT,
        ...
        frozen = json.loads(
            (ROOT / "results/routing_speed_limit/certificate.json").read_text(encoding="utf-8")
        )
    ...
    pdf = ROOT / "paper_routing_speed_limit/every_spectral_switch_costs_memory.pdf"
    if not pdf.read_bytes().startswith(b"%PDF-"):
        raise RuntimeError("paper artifact is not a PDF")
    check_hashes(manifest["artifacts"])
    print("PASS robust spectral-routing release gate")
