...
| C1 | Tangential degree-`N` matrix-polynomial filtering admits an exponential-in-`N` separation between real-symmetric noncommuting coefficients and pairwise-commuting real-symmetric coefficients already at fixed dimension `d=3`. | **Proved; independently reviewed 7/10; priority candidate** | The sharpened construction uses `M=N+3` signatures split among three rational node grids.  The diagonal base has leakage `C0 exp(-cN)` with explicit `c>0`; the symmetric correction is bounded by `exp(-N^3+O(N log N))`.  Standard-library replay checks the conservative envelopes, while the full-spark quadratic witness is exactly certified at `301/304<1`.  The exact revised PDF received 7/10 scientific and 7/10 manuscript, with no proof defect identified.  The theorem is still not licensed as unqualified novelty because two closest full texts remain access-blocked. |
| C2 | At fixed latency, reciprocal linear-phase MIMO FIR filters can have exponential high-frequency advantage over every fixed orthogonal bank of scalar linear-phase FIRs under the same full-spark directional calibrations. | **Corollary proved; independently reviewed; measured-data and modal-task gates passed** | The affine cosine substitution is invertible and preserves coefficient commutation; the exact degree-two witness becomes a rational palindromic five-tap certificate.  For commuting comparators the constructed gap survives calibration residuals `delta < 3/32300`.  A separate VBL-VA001 triaxial calibration has an exact rational `<24/25` versus `1` continuum certificate and `0.0038491` maximum held-out directional residual.  The FDD invariance lemma shows exact preservation of rank-one modal components; held-out filtering yields at most `0.2217` degrees of leading-direction drift and `2.40e-5` relative leading-ordinate change.  Independent review found no proof defect; restricted-full-text priority uncertainty and the absence of deployment evidence remain explicit. |
