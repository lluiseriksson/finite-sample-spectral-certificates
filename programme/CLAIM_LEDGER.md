...
| C1 | Tangential degree-`N` matrix-polynomial filtering admits an exponential-in-`N` separation between irreducible real-symmetric coefficients and every reducible real-symmetric coefficient family already at fixed dimension `d=3`. | **Proved with exponential-scale near-reducibility robustness; priority candidate** | The comparator strictly contains the pairwise-commuting class and permits a noncommuting `2 x 2` block.  The construction uses `M=N+3`, rational `epsilon_N=2^(-13N)`, full-spark margin at least `2^(-39N)/(48(N+4)^6)`, and an `exp(O(N))` symmetric interpolation inverse.  Leakage remains at least `3/4` when calibration residual plus sampled off-block coupling is below an explicit `exp(-O(N))` radius.  A universal scalar filter proves constant tolerance impossible.  Lean kernel-checks the scalar root-count core; the full theorem remains conventionally proved.  Fresh review is pending; two closest full texts remain access-blocked. |
| C2 | At fixed latency, reciprocal linear-phase MIMO FIR filters can have exponential high-frequency advantage over every reducible symmetric three-channel architecture under the same full-spark directional calibrations. | **Robust corollary proved; stronger exact certificate; measured-data gate passed** | The affine cosine substitution transfers invariant subspaces and the quantitative off-block defect.  The rational five-tap certificate has continuum leakage `25/32`, lower bound `1-60(delta+beta)` against near-reducible comparators, and strict reducible gap for `delta < 7/1920`.  A separate VBL-VA001 triaxial calibration has an exact rational `<24/25` versus `>=1` certificate for the full reducible symmetric class and `0.0038491` maximum held-out residual.  Restricted-full-text priority uncertainty and the absence of deployment evidence remain explicit. |

## Current headline under test

> At fixed latency, reciprocal linear-phase MIMO FIR filters can exhibit an
> exponential stopband advantage over every reducible symmetric three-channel
> FIR architecture while preserving the same full-spark directional calibrations.
