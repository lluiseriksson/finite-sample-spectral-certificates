...
| C1 | Tangential degree-`N` matrix-polynomial filtering admits an exponential-in-`N=d` separation between real-symmetric noncommuting coefficients and pairwise-commuting real-symmetric coefficients. | **Proved internally; priority candidate** | The sharpened construction needs only `d=N` channels and `M=2N` signatures.  The full-spark quadratic witness is exactly replayed with continuum bound `301/304<1`; the asymptotic proof fixes the norms and gives correction `8019000 N^(9/2) exp(-N^3+N DeltaEta)`.  The theorem is not yet licensed as novel: full-text collision checks remain open. |
