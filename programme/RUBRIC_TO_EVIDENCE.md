...
| Optimality only for scalar filters | Degree ablation does not address matrix-filter optimality | Either solve a genuinely matrix-valued minimax problem with a noncommutative advantage, or explicitly discard this route after collision/pilot audit | **Mathematically and task-wise closed for Route C:** asymptotic theorem, exact five-tap witness, robust commuting bound, measured VBL-VA001 `<24/25` versus `1` certificate, exact FDD modal-invariance lemma and held-out cospectral replay; priority audit remains open |
...
| Numerical evidence could be solver-dependent | Representative exact-rational dual replay exists | Exact/interval proof must cover the new headline constant or lemma | **Closed for both Route-C finite witnesses:** rational Bernstein replay for `25/32`, robust slope `60`, and radius `7/1920`; rational grid-plus-derivative replay for the measured `<24/25` certificate |
